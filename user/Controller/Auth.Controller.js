...
module.exports = {
  register: async (req, res, next) => {
    try {
      ...
      if (doesExist)
        throw createError.Conflict(`Mobile is already been registerd `);
  ...
  getDataById: async (req, res, next) => {
    try {
      ...
      if (result) {
        res.send({ success: true, msg: "Detail Fetched", data: userDataSend });
      } else {
  ...
  forgetPapersetterPaasword: async (req, res, next) => {
    try {
      ...
      updatedUser = await User.findByIdAndUpdate(
        ...
        { $set: { is_password_reset: true, ...result } }
      );
  ...
  validatePin: async (req, res, next) => {
    try {
      ...
      if (!isMatch) throw createError.NotAcceptable("Pin not valid");
    ...
    try {
      ...
      if (isMatch) {
        user = await User.updateOne(
          ...
          { $set: { spin: "", spin_enable: false } }
        );
        res.send({
          success: true,
          msg: "Authentication Successfull, MPIN reset done",
          data: true,
        });
      }
    } catch (error) {
