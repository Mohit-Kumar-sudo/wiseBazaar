...
@Component({
  selector: 'app-tripple-patti',
  templateUrl: './tripple-patti.component.html',
  styleUrls: ['./tripple-patti.component.css'],
})
export class TripplePattiComponent implements OnInit {
  params: any;
  today = Date.now();
  userInputType: any;
  userId: any;
  ...
  bidCreate(frm: NgForm) {
    ...
    let dbp = frm.value.tripplePatti;
    dbp = dbp.split('');
    let isdbl = false;
    if (dbp.length == 3) {
      if (dbp[0] == dbp[1] && dbp[1] == dbp[2]) {
        isdbl = true;
      }
    }
    if (!isdbl) {
    ...
    if (currentWalletAmount >= 0) {
      ...
      data = {
        ...
        betType: this.params.type,
        userInput: {
          userInputValue: frm.value.tripplePatti,
          userInputCount: 1,
        },
        userInputType: 'tripple Pattii',
