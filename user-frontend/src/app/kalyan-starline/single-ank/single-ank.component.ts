...
@Component({
  selector: 'app-single-ank',
  templateUrl: './single-ank.component.html',
  styleUrls: ['./single-ank.component.css'],
})
export class SingleAnkComponent implements OnInit {
  params: any;
  userInputType: any;
  userId: any;
  userWallet: any;
  ...
  bidCreate(frm: NgForm) {
    ...
    if (currentWalletAmount >= 0) {
      ...
      data = {
        ...
        eventId: this.params.id,
        betType: this.params.title,
        userInput: { singleDigit: frm.value.singleDigit, userInputCount: 1 },
