...
const Model = require('../models/Withdrawal.Model')
...
module.exports = {
    ...
    updateById: async (req, res, next) => {
        try {
            ...
            if (data.status == 'Failed' && data.status != 'Approved') {
                let result = await UserModel.updateOne({ _id: mongoose.Types.ObjectId(data.uid) }, { $inc: { wallet: data.payAmount } })
            }
            let result2 = {}
            if (result2) {
